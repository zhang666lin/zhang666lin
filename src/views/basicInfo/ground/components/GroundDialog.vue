    ...
    <div class="dialog-content q-px-md">
      ...
      <q-form ref="contactFormRef" autocomplete="off">
        <div class="row justify-between items-center q-py-sm">
          ...
          <div class="col-2 text-center">{{ t('common.destination') }}</div>
          <div class="col-2 text-center">{{ t('baseInfo.postCode') }}</div>
          <div class="col-1 text-center">{{ t('baseInfo.name') }}</div>
...
import { addGround, editGround } from '@/api/basicInfo/ground'
...
const emit = defineEmits(['closeGroundDialog'])
  ...
  if (baseInfoFormValidStatus && contactFormValidStatus) {
    ...
    try {
      if (props.operateType === 'add') {
        res = await addGround(params)
      } else {
        res = await editGround(params, props.row.id)
      }
      if (res.code === 200) {
        showSuccess(res.msg)
        return true
...
function handleEdit() {
  const { name, destination, is_enable, contact_person } = props.row

  baseInfoForm.value = { name, destination, is_enable: is_enable.toString() }
  contactForm.value = contact_person.map(contact => {
    const { destination, email, ground_staff_id, id, name, phone, post_code } =
      contact
    return {
      destination,
      email,
