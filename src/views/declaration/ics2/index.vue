<template>
  <div class="fit overflow-auto column no-wrap">
    <Form ref="formRef" class="q-mb-md" :schemas="formSchemas" @submit="search">
      <template #suffix>
        <q-btn
          v-auth="'ics2ManifestBatchDownload'"
          no-caps
          color="primary"
  ...
  <!-- 详情 -->
  <OrderDetail ref="orderDetail" source="ics2" />
</template>
...
defineOptions({ name: 'Ics2' })

const { t } = useI18n()
const { ics2FormSchemas, tableColumns } = useICS2()

const formSchemas = ref(ics2FormSchemas)
...
async function handleDownload() {
  ...
  handleDownloadDeclareTableList('ics2', successDownloadTableCallback, params)
}
...
function getICS2List({ page, rowsPerPage }) {
  let params = handleFetchParams()

  return new Promise((resolve, reject) => {
    getOrderListApi(params, { page, size: rowsPerPage, service_type: 'ics2' })
      .then(res => {
